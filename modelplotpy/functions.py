...
def plot_response(plot_input, save_fig=True, save_fig_filename='', highlight_decile=False, highlight_how='plot_text'):
    ...
    if scope == "no_comparison":
        ax.set_title("model: %s & dataset: %s & target class: %s" % (models[0], datasets[0], classes[0]),
                     fontweight='bold')
        ax.plot(plot_input.decile, plot_input.pct, label=classes[0], color=colors[0])
        ax.plot(plot_input.decile, plot_input.pct_ref, linestyle='dashed', label="overall response (%s)" % classes[0],
                color=colors[0])
        ax.legend(loc='upper right', shadow=False, frameon=False)
    elif scope == "compare_datasets":
        for col, i in enumerate(datasets):
            ax.plot(plot_input.decile[plot_input.dataset_label == i], plot_input.pct[plot_input.dataset_label == i],
                    label=i, color=colors[col])
            ax.plot(plot_input.decile[plot_input.dataset_label == i], plot_input.pct_ref[plot_input.dataset_label == i],
                    linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
        ax.set_title("scope: comparing datasets & model: %s & target class: %s" % (models[0], classes[0]),
                     fontweight='bold')
        ax.legend(loc='upper right', shadow=False, frameon=False)
    elif scope == "compare_models":
        for col, i in enumerate(models):
            ax.plot(plot_input.decile[plot_input.model_label == i], plot_input.pct[plot_input.model_label == i],
                    label=i, color=colors[col])
            ax.plot(plot_input.decile[plot_input.model_label == i], plot_input.pct_ref[plot_input.model_label == i],
                    linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
        ax.set_title("scope: comparing models & dataset: %s & target class: %s" % (datasets[0], classes[0]),
                     fontweight='bold')
        ax.legend(loc='upper right', shadow=False, frameon=False)
    else:  # compare_targetclasses
        for col, i in enumerate(classes):
            ax.plot(plot_input.decile[plot_input.target_class == i], plot_input.pct[plot_input.target_class == i],
                    label=i, color=colors[col])
            ax.plot(plot_input.decile[plot_input.target_class == i], plot_input.pct_ref[plot_input.target_class == i],
                    linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
        ax.set_title("Scope: comparing target classes & dataset: %s & model: %s" % (datasets[0], models[0]),
                     fontweight='bold')
        ax.legend(loc='upper right', shadow=False, frameon=False)
...
def plot_cumresponse(plot_input, save_fig=True, save_fig_filename='', highlight_decile=False,
    ...
    if scope == "no_comparison":
        ...
        for col, i in enumerate(datasets):
            ax.plot(plot_input.decile[plot_input.dataset_label == i], plot_input.cumpct[plot_input.dataset_label == i],
                    label=i, color=colors[col])
            ax.plot(plot_input.decile[plot_input.dataset_label == i], plot_input.pct_ref[plot_input.dataset_label == i],
                    linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
        ax.set_title("scope: comparing datasets & model: %s & target class: %s" % (models[0], classes[0]),
        ...
        for col, i in enumerate(models):
            ax.plot(plot_input.decile[plot_input.model_label == i], plot_input.cumpct[plot_input.model_label == i],
                    label=i, color=colors[col])
            ax.plot(plot_input.decile[plot_input.model_label == i], plot_input.pct_ref[plot_input.model_label == i],
                    linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
        ax.set_title("Scope: comparing models & dataset: %s & target class: %s" % (datasets[0], classes[0]),
        ...
        for col, i in enumerate(classes):
            ax.plot(plot_input.decile[plot_input.target_class == i], plot_input.cumpct[plot_input.target_class == i],
                    label=i, color=colors[col])
            ax.plot(plot_input.decile[plot_input.target_class == i], plot_input.pct_ref[plot_input.target_class == i],
                    linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
...
def plot_cumlift(plot_input, save_fig=True, save_fig_filename='', highlight_decile=False, highlight_how='plot_text'):
    ...
    if scope == "no_comparison":
        ...
        for col, i in enumerate(datasets):
            ax.plot(plot_input.decile[plot_input.dataset_label == i], plot_input.cumlift[plot_input.dataset_label == i],
                    label=i, color=colors[col])
        ...
        for col, i in enumerate(models):
            ax.plot(plot_input.decile[plot_input.model_label == i], plot_input.cumlift[plot_input.model_label == i],
                    label=i, color=colors[col])
        ...
        for col, i in enumerate(classes):
            ax.plot(plot_input.decile[plot_input.target_class == i], plot_input.cumlift[plot_input.target_class == i],
                    label=i, color=colors[col])
...
def plot_cumgains(plot_input, save_fig=True, save_fig_filename='', highlight_decile=False, highlight_how='plot_text'):
    ...
    ax.yaxis.set_ticks_position('left')
    ax.xaxis.set_ticks_position('bottom')
    ax.plot(list(range(0, 11, 1)), np.linspace(0, 1, num=11).tolist(), linestyle='dashed', label="minimal gains",
            color='grey')
    ax.grid(True)
    ax.set_xlim([0, 10])
    ax.set_ylim([0, 1])
    ...
    if scope == "no_comparison":
        ...
        ax.plot(plot_input.decile, plot_input.gain_opt, linestyle='dashed', label="optimal gains (%s)" % classes[0],
                color=colors[0], linewidth=1.5)
        ax.legend(loc='lower right', shadow=False, frameon=False)
    elif scope == "compare_datasets":
        for col, i in enumerate(datasets):
            ax.plot(plot_input.decile[plot_input.dataset_label == i], plot_input.cumgain[plot_input.dataset_label == i],
                    label=i, color=colors[col])
            ax.plot(plot_input.decile[plot_input.dataset_label == i],
                    plot_input.gain_opt[plot_input.dataset_label == i], linestyle='dashed',
                    label="optimal gains (%s)" % i, color=colors[col], linewidth=1.5)
        ax.set_title("scope: comparing datasets & model: %s & target class: %s" % (models[0], classes[0]),
                     fontweight='bold')
        ax.legend(loc='lower right', shadow=False, frameon=False)
    elif scope == "compare_models":
        for col, i in enumerate(models):
            ax.plot(plot_input.decile[plot_input.model_label == i], plot_input.cumgain[plot_input.model_label == i],
                    label=i, color=colors[col])
            ax.plot(plot_input.decile[plot_input.model_label == i], plot_input.gain_opt[plot_input.model_label == i],
                    linestyle='dashed', label="optimal gains (%s)" % i, color=colors[col], linewidth=1.5)
        ax.set_title("scope: comparing models & dataset: %s & target class: %s" % (datasets[0], classes[0]),
                     fontweight='bold')
        ax.legend(loc='lower right', shadow=False, frameon=False)
    else:  # compare_targetclasses
        for col, i in enumerate(classes):
            ax.plot(plot_input.decile[plot_input.target_class == i], plot_input.cumgain[plot_input.target_class == i],
                    label=i, color=colors[col])
            ax.plot(plot_input.decile[plot_input.target_class == i], plot_input.gain_opt[plot_input.target_class == i],
                    linestyle='dashed', label="optimal gains (%s)" % i, color=colors[col], linewidth=1.5)
        ax.set_title("scope: comparing target classes & dataset: %s & model: %s" % (datasets[0], models[0]),
                     fontweight='bold')
        ax.legend(loc='lower right', shadow=False, frameon=False)
...
def plot_all(plot_input, save_fig=True, save_fig_filename=''):
    ...
    ax1.plot(list(range(0, 11, 1)), np.linspace(0, 1, num=11).tolist(), linestyle='dashed', label="minimal gains",
             color='grey')
    ...
    if scope == "no_comparison":
        ...
        ax1.plot(plot_input.decile, plot_input.gain_opt, linestyle='dashed', label="optimal gains (%s)" % classes[0],
                 color=colors[0])
        ...
        ax3.plot(plot_input.decile, plot_input.pct_ref, linestyle='dashed', label="overall response (%s)" % classes[0],
                 color=colors[0])
        ...
        ax4.plot(plot_input.decile, plot_input.pct_ref, linestyle='dashed', label="overall response (%s)" % classes[0],
                 color=colors[0])
        ...
        for col, i in enumerate(datasets):
            ax1.plot(plot_input.decile[plot_input.dataset_label == i],
                     plot_input.cumgain[plot_input.dataset_label == i], label=i, color=colors[col])
            ax1.plot(plot_input.decile[plot_input.dataset_label == i],
                     plot_input.gain_opt[plot_input.dataset_label == i], linestyle='dashed',
                     label="optimal gains (%s)" % i, color=colors[col])
            ax2.plot(plot_input.decile[plot_input.dataset_label == i],
                     plot_input.cumlift[plot_input.dataset_label == i], label=i, color=colors[col])
            ax3.plot(plot_input.decile[plot_input.dataset_label == i], plot_input.pct[plot_input.dataset_label == i],
                     label=i, color=colors[col])
            ax3.plot(plot_input.decile[plot_input.dataset_label == i],
                     plot_input.pct_ref[plot_input.dataset_label == i], linestyle='dashed',
                     label="overall response (%s)" % i, color=colors[col])
            ax4.plot(plot_input.decile[plot_input.dataset_label == i], plot_input.cumpct[plot_input.dataset_label == i],
                     label=i, color=colors[col])
            ax4.plot(plot_input.decile[plot_input.dataset_label == i],
                     plot_input.pct_ref[plot_input.dataset_label == i], linestyle='dashed',
                     label="overall response (%s)" % i, color=colors[col])
        ax1.legend(loc='lower right', shadow=False, frameon=False)
        ax2.legend(loc='upper right', shadow=False, frameon=False)
        ...
        for col, i in enumerate(models):
            ax1.plot(plot_input.decile[plot_input.model_label == i], plot_input.cumgain[plot_input.model_label == i],
                     label=i, color=colors[col])
            ax1.plot(plot_input.decile[plot_input.model_label == i], plot_input.gain_opt[plot_input.model_label == i],
                     linestyle='dashed', label="optimal gains (%s)" % i, color=colors[col])
            ax2.plot(plot_input.decile[plot_input.model_label == i], plot_input.cumlift[plot_input.model_label == i],
                     label=i, color=colors[col])
            ax3.plot(plot_input.decile[plot_input.model_label == i], plot_input.pct[plot_input.model_label == i],
                     label=i, color=colors[col])
            ax3.plot(plot_input.decile[plot_input.model_label == i], plot_input.pct_ref[plot_input.model_label == i],
                     linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
            ax4.plot(plot_input.decile[plot_input.model_label == i], plot_input.cumpct[plot_input.model_label == i],
                     label=i, color=colors[col])
            ax4.plot(plot_input.decile[plot_input.model_label == i], plot_input.pct_ref[plot_input.model_label == i],
                     linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
        ax1.legend(loc='lower right', shadow=False, frameon=False)
        ax2.legend(loc='upper right', shadow=False, frameon=False)
        ax3.legend(loc='upper right', shadow=False, frameon=False)
        ...
        for col, i in enumerate(classes):
            ax1.plot(plot_input.decile[plot_input.target_class == i], plot_input.cumgain[plot_input.target_class == i],
                     label=i, color=colors[col])
            ax1.plot(plot_input.decile[plot_input.target_class == i], plot_input.gain_opt[plot_input.target_class == i],
                     linestyle='dashed', label="optimal gains (%s)" % i, color=colors[col])
            ax2.plot(plot_input.decile[plot_input.target_class == i], plot_input.cumlift[plot_input.target_class == i],
                     label=i, color=colors[col])
            ax3.plot(plot_input.decile[plot_input.target_class == i], plot_input.pct[plot_input.target_class == i],
                     label=i, color=colors[col])
            ax3.plot(plot_input.decile[plot_input.target_class == i], plot_input.pct_ref[plot_input.target_class == i],
                     linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
            ax4.plot(plot_input.decile[plot_input.target_class == i], plot_input.cumpct[plot_input.target_class == i],
                     label=i, color=colors[col])
            ax4.plot(plot_input.decile[plot_input.target_class == i], plot_input.pct_ref[plot_input.target_class == i],
                     linestyle='dashed', label="overall response (%s)" % i, color=colors[col])
        ax1.legend(loc='lower right', shadow=False, frameon=False)
        ax2.legend(loc='upper right', shadow=False, frameon=False)
        ax3.legend(loc='upper right', shadow=False, frameon=False)
...
class modelplotpy(object):
        ...
        for i in range(len(self.models)):
            ...
            for j in range(len(self.dataset_labels)):
                ...
                dataset = pd.concat([self.feature_data[j], probabilities, y_true], axis=1,
                                    join_axes=[self.feature_data[j].index])
                ...
                for k in self.models[i].classes_:
                    ...
                    dataset["dec_" + k] = 10 - pd.DataFrame(pd.qcut(prob_plus_smallrandom, 10, labels=False),
    ...
    def aggregate_over_deciles(self):
        ...
        deciles_aggregate = pd.concat([add_origin, deciles_aggregate], axis=0).sort_values(
            by=['model_label', 'dataset_label', 'target_class', 'decile'])
        cols = deciles_aggregate.columns
        return deciles_aggregate[cols]

    def plotting_scope(self, scope='no_comparison', select_model_label=[], select_dataset_label=[],
                       select_targetclass=[], select_smallest_targetclass=True):
        """ Create plot_input
        ...
        if scope not in ('no_comparison', 'compare_models', 'compare_datasets', 'compare_targetclasses'):
            raise ValueError(
                'Invalid scope value, it must be one of the following: no_comparison, compare_models, compare_datasets or compare_targetclasses.')
